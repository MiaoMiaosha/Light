...
function getEmpsList() {
    var sbox = $('#searchKey'),
        sctn = '<option value="">全部</option>';
    $.get(urlInfo() + '/admin/employee/list?job=1', function(res) {
        $.each(res.data, function(i) {
            sctn += '<option value="' + this.eid + '">' + this.name + '</option>'
        })
        sbox.html(sctn)
    })
}
...
//     //     $('.nextpage').removeClass('d_none');
//     //     if (getStr('page') == 1) {
//     //         $('.prevpage').addClass('d_none');
//     //     } else {
//     //         var page = parseInt(getStr('page')) - 1;
//     //         var employeeId = $('#searchKey').val();
